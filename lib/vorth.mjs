...
const AGY_WINDOWS_INSTALLER = "https://antigravity.google/cli/install.ps1";
const SUPERPOWERS_REPOSITORY = "https://github.com/obra/superpowers";
...
function buildRepairPlan(status) {
  ...
  if (status.superpowers.requirement === "required" && status.superpowers.health !== "healthy") {
    if (!status.superpowers.providers.agyPlugin) {
      const agyCli = detectAgyCli().status === "detected";
      const canInstallCli = process.platform === "win32";
      add({ stack: "superpowers", target: "agy", scope: "harness", network: true, native: true, fixable: agyCli || canInstallCli, reason: "official Antigravity Superpowers runtime is missing", action: agyCli ? "Install the official Antigravity plugin." : canInstallCli ? "Install the official Antigravity CLI, then install the official Superpowers plugin." : "Install the Antigravity CLI, then run the official Superpowers plugin installer." });
    }
...
function detectSuperpowers(repoRoot, desired) {
  ...
  const agyConfigPlugin = findNamedDirectory(path.join(home, ".gemini", "config", "plugins"), "superpowers", 4);
  const providers = {
    projectVendor: fs.existsSync(projectSkills),
    agyPlugin: Boolean(agyConfigPlugin) || anyExists([
...
function setupSuperpowers(repo, options) {
  ...
  if (!isTrue(options.allowNetwork) || !isTrue(options.allowNative) || !isTrue(options.confirm)) {
    return {
      ...
      target,
      status: "approval_required",
      message: "Official Superpowers activation is harness-level. Re-run with --allow-network --allow-native --confirm after reviewing the scope."
    };
  }
  if (target === "agy" || target === "antigravity") {
    let agyCommand = resolveAgyCommand();
    let cliInstall = null;
    if (!agyCommand) {
      if (process.platform !== "win32") {
        return { stack: "superpowers", target: "agy", status: "missing_cli", source: "https://antigravity.google/docs/cli-getting-started" };
      }
      const install = spawnSync("powershell.exe", [
        "-NoProfile", "-ExecutionPolicy", "Bypass", "-Command",
        `Invoke-RestMethod -Uri ${quotePowerShellLiteral(AGY_WINDOWS_INSTALLER)} | Invoke-Expression`
      ], {
        cwd: repo.root,
        encoding: "utf8",
        windowsHide: true,
      ...
      cliInstall = externalResult(install, "installed");
      if (cliInstall.status === "error") return { stack: "superpowers", target: "agy", cliInstall, status: "error" };
      agyCommand = resolveAgyCommand();
      if (!agyCommand) {
        return { stack: "superpowers", target: "agy", cliInstall, status: "error", message: "Antigravity CLI installed, but `agy` is not visible at its documented Windows location. Restart the terminal and run `vorth repair`." };
      }
    }
    const result = spawnWindowsAware(agyCommand, ["plugin", "install", SUPERPOWERS_REPOSITORY], {
      cwd: repo.root,
      timeout: 180000,
      maxBuffer: 10 * 1024 * 1024
    });
    return { stack: "superpowers", target: "agy", ...(cliInstall ? { cliInstall } : {}), ...externalResult(result, "installed") };
  }
  if (target === "codex") {
    return {
      stack: "superpowers",
...
function detectAgyCli() {
  ...
  return command ? detectCli(command) : { status: "missing" };
}

function resolveAgyCommand() {
  if (process.platform !== "win32") return resolveWindowsCommand("agy") ? "agy" : null;
  const fromPath = resolveWindowsCommand("agy");
  if (fromPath) return fromPath;
  const localAppData = process.env.LOCALAPPDATA || path.join(os.homedir(), "AppData", "Local");
  for (const name of ["agy.exe", "agy.cmd", "agy.bat"]) {
    const candidate = path.join(localAppData, "agy", "bin", name);
