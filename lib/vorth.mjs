...
function buildRepairPlan(status) {
  ...
  if (status.ecc.codex.requirement === "required" && status.ecc.codex.health !== "healthy") {
    add({ stack: "ecc", target: "codex", scope: "harness", network: true, native: true, fixable: false, reason: "ECC Codex specialists are missing", action: "Install the official ECC Codex plugin/sync assets, then restart Codex." });
  }
  if (status.codegraph.requirement === "required" && status.codegraph.health !== "healthy") {
    ...
    add({ stack: "codegraph", scope: "mixed", network: cliMissing, native: cliMissing, wire: true, fixable: true, reason: "CodeGraph CLI, index, or local harness wiring is incomplete", action: "Install the official CLI when needed, initialize the index, and wire local agents." });
  }
  ...
  if (status.agyNativeBridge.requirement === "required" && status.agyNativeBridge.health === "configured-unprobed") {
    add({ stack: "bridge", target: "agy", scope: "login", network: false, native: false, fixable: false, reason: "bridge is configured but the worker OAuth session is not verified", action: "Run `vorth bridge login --repo .`, authenticate the worker account, then run `vorth doctor --probe`." });
  }
...
function detectBridgeRegistration(repoRoot) {
  ...
  try {
    ...
    result.legacyRegistered = args.some((arg) => samePath(arg, legacyServer));
    result.status = result.registered ? "registered" : result.legacyRegistered ? "legacy_registration" : "registered_different_path";
...
function getAgyMcpConfigPath() {
  return path.resolve(process.env.VORTH_AGY_MCP_CONFIG || path.join(os.homedir(), ".gemini", "config", "mcp_config.json"));
}
...
function spawnAntigravityAddMcp(command, definition, cwd) {
  ...
  if (process.platform !== "win32") return spawnSync(command, ["--add-mcp", definition], options);
  const script = `& ${quotePowerShellLiteral(command)} '--add-mcp' ${quotePowerShellLiteral(definition)}`;
  return spawnSync("powershell.exe", ["-NoProfile", "-Command", script], options);
}
